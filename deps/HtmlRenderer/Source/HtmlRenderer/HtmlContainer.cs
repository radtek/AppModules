...
namespace HtmlRenderer
{
    ...
    /// The class allows html layout and rendering without association to actual winforms control, thouse allowing to handle html rendering on any graphics object.<br/>
    /// Using this class will require the client to handle all propogation of mouse\keyboard events, layout/paint calls, scrolling offset, 
    ...
    public sealed class HtmlContainer : IDisposable
    {
        ...
        public void PerformLayout(Graphics g)
        {
            ...
            if (_root != null)
            {
                ...
                {
                    // in case the width is not restricted we need to double layout, first will find the width so second can layout by it (center alligment)
                    _root.Size = new SizeF(_actualSize.Width + 1, 0);
                    _actualSize = SizeF.Empty;
                    _root.PerformLayout(g);
                }
            }
